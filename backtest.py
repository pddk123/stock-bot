...
TIME_CUT_DAYS = 5      # 5일 안에 안 오르면 교체 (기회비용)
...
def run_backtest():
    print(f"🚀 v8.0 퀀트 엔진 백테스트: {START_DATE} ~ {END_DATE}")
    ...
    for i in range(len(trading_days) - 1):
        ...
        for symbol, pos in positions.items():
            ...
            # ATR 기반 동적 손절선 (진입가 - 2*ATR)
            stop_price = pos['entry_price'] - (2 * pos['entry_atr'])
            
            sell_trigger = False
            if profit_rate >= TARGET_PROFIT: sell_trigger = True
            elif curr['Close'] < stop_price: sell_trigger = True # ATR 손절
            elif hold_days >= TIME_CUT_DAYS and profit_rate < 0.02: sell_trigger = True # 5일간 노잼이면 매도
        ...
        # [2] 매수 로직 (상대적 모멘텀 상위 20% 필터)
        if len(positions) < MAX_POSITIONS:
            candidates = []
            for symbol, df in all_data.items():
                if symbol not in positions and today in df.index:
                    row = df.loc[today]
                    # 조건: 거래대금 300억 + 정배열 + 모멘텀 양수
                    if row['Amount'] >= 300 and row['Close'] > row['MA20'] and row['Momentum'] > 0:
