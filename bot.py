...
def analyze_stock(symbol, name, multiplier, is_portfolio):
    """v5.6 핵심 분석 로직 (5단계 상태 진단 포함)"""
    try:
        ...
        for i in range(3):
            ...
            if i == 0:
                ...
                if is_portfolio: # 동근 님의 5단계 진단 로직
...
# --- [3. 메인 실행부] ---
def main():
    logger.info("Smart Picking v5.6 실행")
    ...
    # [중요] 중복 제거 및 포트폴리오 우선순위 부여
    target_dict = {c: True for c in my_codes}
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for code, is_port in target_dict.items():
            ...
            if not match.empty:
                tasks.append(executor.submit(analyze_stock, code, match['Name'].iloc[0], vol_multiplier, is_port))
